...
public class TwitterOAuthFlowService {
	...
	public static String init(HttpServletRequest req) {
		...
		OAuth1AuthorizationFlow flow = OAuth1ClientSupport.builder(credentials)
				...
				.callbackUri(callbackHost).build();

		String authorizationUri = flow.start();

		String token = authorizationUri
				.substring(authorizationUri.indexOf(OAUTH_TOKEN_FIELD) + OAUTH_TOKEN_FIELD.length() + 1);
	...
	public static String reissueAuthorization(HttpServletRequest req, HttpServletResponse resp) {
		...
		Cookie accessTokenSecretCookie = new Cookie(TwitterLoginFilter.TOKEN_COOKIE_SECRET,
